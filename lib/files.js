...
function scaffoldMemoryBank(pkgRoot, targetDir) {
  ...
  safe('Create decisions.md', () =>
    fs.writeFileSync(path.join(mbDir, 'decisions.md'), `# Architectural Decisions
...
`)
  ...
  // Scaffold toc.md
  safe('Create toc.md', () =>
    fs.writeFileSync(path.join(mbDir, 'toc.md'), `# Memory Bank — Table of Contents
...
`)
  ...
  safe('Create monthly task README', () =>
    fs.writeFileSync(path.join(tasksDir, 'README.md'), `# ${yyyy}-${mm} — Task Summary
